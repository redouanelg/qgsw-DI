...
def qrhs(u,v,q,grd,way):
    ...
    #diffusion
    if grd.snu is not None:
        rq[2:-2,2:-2]=rq[2:-2,2:-2]+grd.snu/(grd.dx[2:-2,2:-2]**2)*(q[2:-2,3:-1]+q[2:-2,1:-3]-2*q[2:-2,2:-2]) \
        +grd.snu/(grd.dy[2:-2,2:-2]**2)*(q[3:-1,2:-2]+q[1:-3,2:-2]-2*q[2:-2,2:-2])
...
def qrhs_tgl(du,dv,dq,u,v,q,grd,way):
    ...
    #diffusion
    if grd.snu is not None:
        drq[2:-2,2:-2]=drq[2:-2,2:-2]+grd.snu/(grd.dx[2:-2,2:-2]**2)*(dq[2:-2,3:-1]+dq[2:-2,1:-3]-2*dq[2:-2,2:-2]) \
        +grd.snu/(grd.dy[2:-2,2:-2]**2)*(dq[3:-1,2:-2]+dq[1:-3,2:-2]-2*dq[2:-2,2:-2])

        drq[numpy.where((grd.mask<=1))]=0

    return drq,

def qrhs_adj(adrq,u,v,q,grd,way):
    ...
    #diffusion
    if grd.snu is not None:
        adq[2:-2,2:-2]=adq[2:-2,2:-2]+grd.snu/(grd.dx[2:-2,2:-2]**2)*(adrq[2:-2,3:-1]+adrq[2:-2,1:-3]-2*adrq[2:-2,2:-2]) \
        +grd.snu/(grd.dy[2:-2,2:-2]**2)*(adrq[3:-1,2:-2]+adrq[1:-3,2:-2]-2*adrq[2:-2,2:-2]) 
